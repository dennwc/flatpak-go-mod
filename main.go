...
func run(out, pref, lpref, path string) error {
	...
	for _, m := range mods {
		...
		yamlData = append(yamlData, map[string]interface{}{
			"type":             "archive",
	...
	if *fJSON {
		...
		if _, err := y.WriteString(fmt.Sprintf("%vWorkaround for Go modules generated by github.com/dennwc/flatpak-go-mod\n%v", lpref, lpref)); err != nil {
			return err
		}
